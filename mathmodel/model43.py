...
# 示例数据
land_indices = np.arange(54)
crop_names = ['作物{}'.format(i) for i in range(82)]
X, Y = np.meshgrid(land_indices, np.arange(len(crop_names)))
X = X.flatten()
Y = Y.flatten()
Z = np.random.rand(len(X))  # 随机生成一些数据
...
# 绘制 3D 柱状图
ax.bar3d(X, Y, np.zeros_like(Z), 1, 1, Z, alpha=0.8)
...
# 设置 y 轴刻度
ax.set_yticks(np.arange(len(crop_names)))
